...
# ---- Agrupamiento de categorías residuales ----
# Reagrupa en "Otros" las categorías cuya frecuencia relativa cae por debajo
# de `umbral` (por defecto 5%). Pensada para usarse sobre un data.table ya
# agregado con columnas categoria/N, tal como lo producen mi_barplot,
# mi_barplot_pct y mi_piechart antes de graficar.
#
# Si los datos originales ya traen una categoría literal con el mismo nombre
# que `etiqueta_otros` (ej. una opción "Otros" del cuestionario), las
# categorías residuales se SUMAN a esa categoría existente en vez de crear
# una fila duplicada (lo cual rompería factor(levels=...) más adelante).
agrupar_residual <- function(grafico_data,
                             umbral = 0.05,         # 5% por defecto
                             etiqueta_otros = "Otros",
                             min_categorias_otros = 2) {  # no agrupar si hay 1 sola categoría chica
  
  total <- sum(grafico_data$N)
  if (total == 0) return(grafico_data)
  
  grafico_data <- copy(grafico_data)
  grafico_data[, categoria := as.character(categoria)]
  
  es_chica <- (grafico_data$N / total) < umbral
  
  # Si ninguna categoría cae bajo el umbral, o si solo hay una candidata
  # (agrupar una sola categoría en "Otros" no aporta nada y puede confundir
  # más que ayudar), devolvemos los datos sin tocar.
  if (sum(es_chica) < min_categorias_otros) {
    return(grafico_data)
  }
  
  # Si TODAS las categorías son chicas (umbral muy alto o muchísimas
  # categorías parejas), no tiene sentido agrupar todo en "Otros": se
  # perdería toda la información. En ese caso, devolvemos sin agrupar.
  if (sum(es_chica) == nrow(grafico_data)) {
    return(grafico_data)
  }
  
  grandes <- grafico_data[!es_chica]
  chicas  <- grafico_data[es_chica]
  
  suma_residual <- sum(chicas$N)
  
  if (etiqueta_otros %in% grandes$categoria) {
    # Ya existe una categoría grande con ese nombre (ej. "Otros" del
    # cuestionario original): sumamos el residual ahí en vez de duplicar
    grandes[categoria == etiqueta_otros, N := N + suma_residual]
    resultado <- grandes
  } else {
    # No existe entre las grandes (puede o no existir entre las chicas,
    # pero en ese caso ya está incluida en suma_residual): creamos la fila
    otros <- data.table(categoria = etiqueta_otros, N = suma_residual)
    resultado <- rbindlist(list(grandes, otros), use.names = TRUE)
  }
  
  resultado[, categoria := as.character(categoria)]
...
mi_barplot <- function(x,
  ...
  # Agrupar categorías residuales en "Otros" antes de ordenar, para que
  # la barra de "Otros" se ubique según su propia frecuencia agregada
  if (agrupar_otros) {
    n_categorias_antes <- nrow(grafico_data)
    grafico_data <- agrupar_residual(grafico_data, umbral = umbral_otros)
    if (nrow(grafico_data) != n_categorias_antes) {
      # Solo si realmente se agrupó algo en "Otros" perdemos el orden
      # original, porque "Otros" no es parte de los niveles preexistentes
      niveles_originales <- NULL
    }
  }
  ...
  } else if (!is.null(niveles_originales)) {
    # Factor sin agrupar: respetamos el orden de niveles original (ej. litros ascendentes)
    grafico_data[, categoria := factor(categoria, levels = niveles_originales)]
    setorder(grafico_data, categoria)
  } else {
    # Character sin agrupar, o con "Otros" agregado: orden alfabético invertido
    # para que la 'A' quede arriba en horizontal
...
mi_barplot_pct <- function(x,
  ...
  # Agrupar categorías residuales en "Otros" antes de calcular el %, para
  # que "Otros" refleje la suma real de las categorías chicas agregadas
  if (agrupar_otros) {
    n_categorias_antes <- nrow(grafico_data)
    grafico_data <- agrupar_residual(grafico_data, umbral = umbral_otros)
    if (nrow(grafico_data) != n_categorias_antes) {
...
mi_piechart <- function(x,
  ...
  # Agrupar categorías residuales en "Otros" antes de ordenar
  if (agrupar_otros) {
    grafico_data <- agrupar_residual(grafico_data, umbral = umbral_otros)
  }
